...
class Mongodb extends DbDbBase
{
        ...
        switch ($action) {
            ...
            case 'create-start':
                if (empty($name) || empty($version) || empty($port)) {
                    ...
                    $output->writeln('e.g.');
                    $output->writeln("composer dbdb:" . self::SERVICE . " $action --db-name=my-mongodb-4 --db-version=4.4.10 --db-port=27017");
                    $output->writeln("composer dbdb:" . self::SERVICE . " $action --db-name=my-mongodb-5 --db-version=5.0.3 --db-port=random");
