...
class Memcached extends DbDbBase
{
    const SERVICE = 'memcached';
        ...
        switch ($action) {
            ...
            case 'create-start':
                if (empty($name) || empty($version) || empty($port)) {
                    ...
                    $output->writeln('e.g.');
                    $output->writeln("composer dbdb:" . self::SERVICE . " $action --db-name=my-memcached-1 --db-version=1.6.31 --db-port=11211");
                    $output->writeln("composer dbdb:" . self::SERVICE . " $action --db-name=my-memcached-random --db-version=1.6.31 --db-port=random");
            ...
            case 'stop':
                if (empty($name)) {
                    ...
                    $output->writeln('e.g.');
                    $output->writeln("composer dbdb:" . self::SERVICE . " $action --db-name=my-memcached-1");
                    return Command::FAILURE;
                }
                return $this->action($action, $input, $output);
                break;
