...
class Postgresql extends DbDbBase
{
        ...
        switch ($action) {
            ...
            case 'create-start-foo':
                if (empty($name) || empty($version) || empty($port)) {
                    ...
                    $output->writeln('e.g.');
                    $output->writeln("composer dbdb:" . self::SERVICE . " $action --db-name=my-pg-124 --db-version=12.4 --db-port=5432");
                    $output->writeln("composer dbdb:" . self::SERVICE . " $action --db-name=my-pg-132 --db-version=13.2 --db-port=random");
